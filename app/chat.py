    ...
    def stream_video():
        with _app.app_context():
            try:
                ...
                yield "data: " + _json.dumps({"stage": "error", "error": f"Video service unavailable: {e}"}) + "\n\n"
...
def animate_image(thread_id):
    ...
    allowed, used, limit = check_rate_limit()
    if not allowed:
        err = _json.dumps({"error": "rate_limited"})
        def err_stream():
            yield "data: " + err + "\n\n"
        return Response(err_stream(), mimetype="text/event-stream")
    ...
    def stream_animate():
        with _app.app_context():
            try:
                # Read image and send as multipart
                ...
                # For multipart upload we can't easily stream SSE
                # So use the animate endpoint which returns SSE
                resp = req_lib.post(
                    gen_url,
                    files=files,
                    data=data_dict,
